...
## 🏆 最新アルゴリズム (v12.0)
**「市場微細構造シミュレーション」モデル**
- **概要**: v11.0のAIハイブリッド戦略に加え、日本株特有の「値幅制限（ストップ高/安）」や「流動性リスク」を厳密にシミュレート。
- **改善点**:
  - **値幅制限対応**: ストップ高での買い、ストップ安での売りを禁止し、現実的な約定を再現。
  - **スリッページ導入**: 全取引に0.1%のスリッページコストを付加。
  - **流動性フィルター**: 売買代金が少ない局面でのエントリーを制限。
- **v11.0実績 (参考)**:
  - **収益率**: +32.44% (1時間足バックテスト実績)
  - **勝率**: 84.1%
  - **プロフィットファクター**: 5.00
  - **特徴**:
    - **AIハイブリッド判定**: Random Forestによる未来予測スコアを導入し、テクニカル指標のダマシを回避
    - **1時間足対応**: 日中の細かい値動きを捉え、エントリー精度を大幅に向上
    - **Let Profits Run**: トレーリングストップを緩和し、大きなトレンドを最後まで追随
    - **Deep Valueスコアリング**: RSI30未満の「売られすぎ」を深度に応じて加点評価
    - **VIXレジーム検知**: 市場環境（PANIC/FEAR）に応じた動的リスク管理
...
| 💼 ポートフォリオ | 保有株管理・損益計算 |
| 🔔 アラート | 価格通知設定 |
| 📊 バックテスト | ウォークフォワード売買シミュレーション |

## 🔄 バックテスト機能 (v9.0c)

過去データを使用したウォークフォワードテストで売買戦略をシミュレーション。

### 主な特徴
- **市場レジーム検知**: 強気/弱気相場の自動判定
- **弱気相場対応**: 自動ポジション縮小・新規購入停止
- **動的損切り**: ATRベース + レジーム調整（最大-10%）
- **早期警戒シグナル**: モメンタム・RSI・MACD複合分析
- **トレーリングストップ**: 高値から-7%で利益確定
